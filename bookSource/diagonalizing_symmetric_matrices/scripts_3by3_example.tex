...
\hypertarget{scripts_diagonalizing_symmetric_matrices_3by3_example}{Let's diagonalize the} matrix
\[
M = 
\begin{pmatrix}
1 &2 & 0 \\
...
Notice that the eigenvectors form an orthogonal basis. We can create an orthonormal basis by rescaling to make them unit vectors. This will help us because if $P = [v_1,v_2,v_3]$ is created from orthonormal vectors then $P^{-1}=P^T$, which means computing $P^{-1}$ should be easy. So let's say 
\[
