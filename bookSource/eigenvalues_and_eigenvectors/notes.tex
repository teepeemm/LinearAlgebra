...
\]
The number $\omega$ is called an angular frequency in many contexts, let's call its square $\lambda:=-\omega^2$ to match notations we will use later (notice that for this particular problem $\lambda$ must then  be negative).
...
%We are aware that we have made the startling suggestion that a derivative operator is a linear combination of products of vectors, and we hope that you are intrigued. Let's now return to the simpler case of linear functions which are matrices to develop this idea.
...
%%\[
%%\colvec{v^1\\ \vdots \\ v^n}\in \mathbb{R}^n, \qquad 
%%\colvec{w^1\\ \vdots \\ w^m}\in \mathbb{R}^m.
%%\]
...
\begin{center}
\includegraphics[alt={A linear transformation L takes e1, e2, and e1+e2 to L(e1), L(e2), and L(e1+e2)=L(e1)+L(e2).  In a second line, L(f1) is parallel to f1, and L(f2) is parallel to f2.},scale=.3]{\eigenPath/invariant.jpg}
\end{center}
...
Now let's try an explicit example to see if we can achieve the last picture:
...
\begin{figure}
\begin{center}
\includegraphics[alt={L(v)=λv where v≠0 is the eigenvector and λ is the eigenvalue.},scale=.4]{\eigenPath/eigeneqn.jpg}
\end{center}
...
L(v)=\lambda v
\]
is called an \emph{eigenvector}\index{Eigenvector} of $L$, and $\lambda$ (which now may be zero) is an \emph{eigenvalue}\index{Eigenvalue}.  Since the direction is all we really care about here, then any other vector $cv$ (so long as $c\neq 0$) is an equally good choice of eigenvector. Notice that the relation ``\(u\) and \(v\) point in the same direction'' is an equivalence relation.
...
\begin{example}
Let $L \colon \mathbb{R}^2\rightarrow \mathbb{R}^2$ such that $L(x,y)=(2x+2y, 16x+6y)$.  First, we  find the matrix of $L$, this is quickest in the standard basis:
\[
...
In section~\ref{eigenvalseigenvects}, we developed the idea of eigenvalues and eigenvectors in the case of linear transformations $\mathbb{R}^2\rightarrow \mathbb{R}^2$.  In this section, we will develop the idea more generally.
...
Lv=\lambda v.
\]
then $\lambda$ is an {\bfseries eigenvalue}\index{Eigenvalue} of $L$ with {\bfseries eigenvector}\index{Eigenvector} $v$.
...
The left hand side of this equation is a polynomial in the variable $\lambda$ called the {\bfseries characteristic polynomial}\index{Characteristic polynomial} $P_M(\lambda)$ of $M$.  For an $n\times n$ matrix, the characteristic polynomial has degree $n$.  Then 
\[
...
\begin{figure}
\begin{center}
\includegraphics[alt={The characteristic polynomial is P_M(λ)=det(λI-M).},scale=.3]{\eigenIIPath/charpoly.jpg}
\end{center}
...
\begin{example}
Let $L$ be the linear transformation $L \colon \mathbb{R}^3\rightarrow \mathbb{R}^3$ given by 
...
\[
\left\{ s\colvec{-1\\1\\0}+t\colvec{1\\0\\1} \middle| s,t\in \mathbb{R}   \right\}.
\]
...
\begin{example}
Let $V$ be the vector space of smooth (\textit{i.e.} infinitely differentiable) functions $f \colon \mathbb{R}\rightarrow \mathbb{R}$.  Then the derivative is a linear operator $\frac{d}{dx} \colon V\rightarrow V$.  What are the eigenvectors of the derivative?  In this case, we don't have a matrix to work with, so we have to make do.
...
The operator $\frac{d}{dx}$ has an eigenvector $e^{\lambda x}$ for every $\lambda \in \mathbb{R}$.
...
The space of all vectors with eigenvalue $\lambda$ is called an {\bfseries eigenspace}\index{Eigenspace}.  It is, in fact, a vector space contained within the larger vector space $V$.  It contains $0_V$, since $L0_V=0_V=\lambda 0_V$, and is closed under addition and scalar multiplication by the above calculation.  All other vector space properties are inherited from the fact that $V$ itself is a vector space. In other words, the \hyperlink{sst}{subspace theorem} (\ref{subspacetheorem}, chapter~\ref{subspacesspanning}) ensures that $V_\lambda:=\{v\in V|Lv=\lambda v\}$ is a subspace of $V$.
...
\begin{center}
\shabox{
{\bfseries \hyperref[sample2]{You can now attempt the second sample midterm. 
\includegraphics[alt={A student working on a midterm.},scale=.15]{midterm.jpg}}}}
\end{center}
%
%\section*{References}
%Hefferon, Chapter Three, Section III.1: Representing Linear Maps with Matrices
