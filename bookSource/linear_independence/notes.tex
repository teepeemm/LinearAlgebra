...
Consider a plane $P$ that includes the origin in $\mathbb{R}^3$ and non-zero vectors $\{u,v,w\}$ in $P$.
\begin{center}
\includegraphics[alt={A plane P with vectors u, v, and w all starting from the point O},scale=.3]{\linIndepPath/span_plane.jpg}
\end{center}
...
\[
c^1u+c^2v+c^3w=0 \qquad c^i \in \mathbb{R}, \text{ some $c^i\neq 0$}
\]
