...
Given column vectors $v$ and $w$, we have seen that the dot product $v\dotprod w$ is the same as the matrix multiplication $v^Tw$.  This is an \emph{inner product}\index{Inner product} on~$\mathbb{R}^n$.  We can also form the \emph{outer product}\index{Outer product} $vw^T$, which gives a square matrix.
...
Suppose $T=\{u_1, \ldots, u_n \}$ is an orthonormal basis for $\mathbb{R}^n$.  Because $T$ is a basis, we can write any vector $v$ uniquely as a linear combination of the vectors in $T$;
\[
...
Suppose $T=\{u_1, \ldots, u_n \}$ and $R=\{w_1, \ldots, w_n \}$ are two orthonormal bases for $\mathbb{R}^n$.  Then\\[3mm]
\begin{align*}
...
\begin{example}
Consider $\mathbb{R}^3$ with the ordered orthonormal basis 
\[
...
This algorithm is called the {\bfseries Gram-Schmidt orthogonalization procedure}\index{Gram-Schmidt orthogonalization procedure}\label{GramSchmidt}---Gram worked at a Danish insurance company over one hundred years ago, Schmidt was a student of Hilbert (the famous German mathematician).
...
\]
is an orthogonal basis for $\mathbb{R}^3$.  To obtain an ortho{\itshape normal} basis we simply divide each of these vectors by its length, yielding
\[
...
In the special case that $U$ and $V$ do not have any non-zero vectors in common, their sum is a vector space with dimension $\dimension U + \dimension V$. 
...
\begin{center}
\includegraphics[alt={A plane U in space W.  The vectors V and V' are not in the plane.},scale=.25]{\gramSchmidtPath/direct_sums.jpg}
\end{center}
However, using the inner product, there is a natural candidate $U^\perp$ for this second subspace as shown below.
\begin{center}
\includegraphics[alt={The same picture, but now U⊥ is drawn in, and W is the direct sum of U and U⊥.},scale=.25]{\gramSchmidtPath/U_perp.jpg}
\end{center}
...
\begin{example}
Consider any line \(L\) through the origin in \(\mathbb{R}^4\). Then \(L\) is a subspace, and \(L^\perp\) is a \(3\)-dimensional subspace orthogonal to \(L\). For example, let 
\[L=\spa \left\{ \colvec{1\\1\\1\\1} \right\}\] 
be a line in \(\mathbb{R}^4.\) Then 
\begin{align*}
L^\perp&=\left \{  \colvec{x\\y\\z\\w} \in \mathbb{R}^4 ~\middle| ~ (x,y,z,w) \dotprod (1,1,1,1)=0 \right\} \\
&=\left\{ \colvec{x\\y\\z\\w} \in \mathbb{R}^4 ~\middle| ~ x+y+z+w=0 \right \}.
\end{align*}
...
Moreover, we have
\[
\mathbb{R}^4=L \oplus L^\perp = 
\left\{ \colvec{c\\c\\c\\c} \middle| c \in \mathbb{R}  \right\} 
\oplus \left\{   \colvec{x\\y\\z\\w} \in \mathbb{R}^4 \middle| \,  x+y+z+w=0\right\},
\]
a decomposition of $\mathbb{R}^4$ into a line and its three dimensional orthogonal complement.
\end{example}
