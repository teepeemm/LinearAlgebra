...
\begin{center}
\includegraphics[alt={A barrel of apples and oranges.},scale=.3]{\diagPath/barrel.jpg}
\end{center}
...
\vspace{2mm}
\begin{center}
\begin{tikzpicture}[alt={A graph with horizontal axis apples and vertical axis oranges.},domain=0:4]
...
\begin{center}
\includegraphics[alt={A barrel of apples and oranges and fruit and sugar, labeled with vectors e1, e2, x, f1, and f2.},scale=.3]{\diagPath/barrel3.jpg}
\end{center}
...
\right.
\]
Essentially, this is already our change of basis formula, but let's play around and put it in our notations.
...
Finally, let's consider the very first linear systems problem, where you were given that there were 27 pieces of fruit in total and twice as many oranges as apples. In equations this says just
\[
...
\]
Also let's call \[ \vec v:= \begin{pmatrix}\vec e_1 & \vec e_2\end{pmatrix}\, V\, .\]
Now the matrix $M$ is the matrix of some linear transformation $L$ in the basis of the engineers.
Let's convert it to the basis of the nutritionists:
\[
...
Thus the last task is to solve the system, let's solve for sugar and fruit. 
We need to solve
\[
