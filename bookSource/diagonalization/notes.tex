...
\shabox{\centering The columns of the change of basis matrix are the components\\ of the new basis vectors  in terms of the old basis vectors.}
...
Let's now calculate how the matrix of a linear transformation changes when changing basis.
...
If for two matrices $N$ and $M$ there exists a matrix $P$ such that $M=P^{-1}NP$, then we say that $M$ and $N$ are {\bfseries similar}\index{Similar matrices}.  Then the above discussion shows that diagonalizable matrices are similar to diagonal matrices.
...
\begin{figure}
\begin{center}
\includegraphics[alt={Theorem: Suppose L:V->V is linear.  Then {v1,...,vn} is a basis of eigenvectors if and only if the matrix of L is diagonal in the basis {v1,...,vn}.},scale=.33]{\diagPath/eigenbasis.jpg}
\end{center}
