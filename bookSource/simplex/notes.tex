...
\vspace{-5mm}
\begin{center}
\hspace{-1cm}\includegraphics[alt={The feasible region graphs the four equations, creating a slanted trapezoid with vertices (8,7), (5,10), (18,7), and (5,20).},scale=.32]{simplex/feasible.jpg}
\end{center}
\end{example}
...
than prove this, let's look at a plot of the linear function $s(x,y)=5x+10y$.
...
To see how this works let's try an example.
...
\begin{figure}
\begin{center}\tagpdfsetup{table/header-rows={3},table/header-columns={2}}
\begin{tabular}{ll}
\ \ %
{$\overbrace{\phantom{VERYVERYERY}}^{\mbox{\tiny variables (incl. slack and artificial)}}$}\ %
{$\overbrace{\phantom{F}}^{\mbox{\tiny objective}}$}&\\ % end row 1
{$
\left(
\begin{array}{c|c}
...
\end{array}
\right)
$} &{$\begin{array}{l} \\ \leftarrow  \mbox{constraint equations} \\[6mm] \leftarrow \mbox{objective equation}\end{array}$}\\ % end row two
\hspace{4.3cm}{$\begin{array}{c}\uparrow\\ \mbox{objective value}\end{array}\!\!\!\!\!\!\!\!$} % end row three
\end{tabular}
\end{center}
...
To see why we are done, let's write out what our row operations have done in terms of the function $f$ and the constraints $(c_1,c_2)$.
First we have
\[
f=16-7y-6z
\]
...
Let's take $\alpha=10$ (our solution will not depend on this choice) so that our augmented matrix reads 
\begin{align*}
&
