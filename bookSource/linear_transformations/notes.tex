...
A linear transformation with domain $\mathbb{R}^3$ is completely specified by the way it acts on the three vectors 
\[
...
\begin{example}\label{Vdef} Let \[V=\left\{  c_1\colvec{1\\1\\0} +c_2\colvec{0\\1\\1} \middle| c_1,c_2\in \mathbb{R} \right\} \] and consider $L:V\to \mathbb{R}^3$ be a linear function that obeys 
\[
...
%This is often called the {\itshape linearity property}\index{Linearity} of a linear transformation.



%\begin{example}
%Take $L \colon \mathbb{R}^3\rightarrow \mathbb{R}^3$ defined by:
%
%\[
%L\colvec{x\\y\\z} = \colvec{ x+y\\y+z\\0 }
%\]
%The domain of $L$ is $\mathbb{R}^3$. The range is not all of $\mathbb{R}^3$, but just the plane comprised of vecotrs whose third component is zero. We say that $L$ transforms $\mathbb{R}^3$ into this plane.\\
%
...
You saw that a linear operator acting on $\mathbb{R}^2$ is completely specified by how it acts on the pair of vectors $\colvec{1\\0}$ and $\colvec{0\\1}$. 
...
%\colvec{a\\b}.
%\]
%but only by virtue of it having domain and targert $\mathbb{R}^2$.
\end{example}

It should not surprise you to learn there are infinitely many pairs of vectors from $\mathbb{R}^2$ 
with the property that any vector can be expressed as a linear combination of them; any pair that when used as columns of a matrix gives an invertible matrix works. Such a pair is called a {\itshape basis}\index{basis} for $\mathbb{R}^2$.
...
%\\
%=\left\{  c_1\colvec{2\\4\\2} +c_2 \colvec{1\\3\\2}  | c_1,c_2\in \mathbb{R} \right\} 
\]
...
%Some of the examples of vector spaces we have worked with have been finite dimensional.  (For example, $\mathbb{R}^n$ will turn out to have dimension $n$.)  
...
A minimal set of independent vectors is called a {\itshape basis}\index{basis} (see Chapter~\ref{basisdimension} for the precise definition). 
...
The fact that every vector space (over~$\mathbb{R}$) has infinitely many bases is actually very useful. 
