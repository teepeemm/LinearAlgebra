...
\hypertarget{non_linear_example}{For a non-linear example} let's take the vector space ${\mathbb R}^1= {\mathbb R}$
with
...
need $L(cu)=cL(u)$ for {\itshape any}~$c$, the game is up! $x\mapsto x+1$ is {\itshape not} a linear transformation.
