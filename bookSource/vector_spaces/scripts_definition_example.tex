...
\hypertarget{scripts_vector_spaces_definition_example}{Let's show that ${\mathbb R}^2$} is a vector space.
...
\]
\item[(+ii)] Additive commutativity: We want to check that when we add any two vectors we can do so in either order, {\itshape i.e.} 
\[
\begin{pmatrix}x_1\\x_2\end{pmatrix}+
\begin{pmatrix}y_1\\y_2\end{pmatrix}
\stackrel?=
...
({\itshape i.e.} numbers) but do NOT multiply a vectors by  vectors.
...
acts the way we would expect ``1'' to behave. {\itshape I.e.} \[
\mbox{``1''}\cdot\begin{pmatrix}x_1\\x_2\end{pmatrix}=\begin{pmatrix}x_1\\x_2\end{pmatrix}\, .
\]
There is an obvious choice for this special scalar---just the real number $1$ itself. Indeed, to be pedantic let's calculate
\[
1\cdot\begin{pmatrix}x_1\\x_2\end{pmatrix}=\begin{pmatrix}1.x_1\\1.x_2\end{pmatrix}=\begin{pmatrix}x_1\\x_2\end{pmatrix}\, .
\]
\end{enumerate}
Now we are done---we have really proven the ${\mathbb R}^2$ is a vector space so let's write a little square $\square$ to celebrate.
