...
Firstly, let's start with the subspace sum. Remember that even if $U$ and $V$ are subspaces, their union $U\cup V$ is usually not a subspace. However, the span of their union certainly is
...
You need to be aware that this is a sum of vector spaces (not vectors). A picture of this is a pair of planes in ${\mathbb R}^3$: 
 \begin{center}\includegraphics[alt={Two intersecting planes U and V.},scale=.3]{\gramSchmidtPath/U+V.jpg}\end{center}
Here $U+V={\mathbb R}^3$. 

Next let's consider a direct sum. This is just the subspace sum for the case when $U\cap V=\{0\}$. For that we can keep the plane $U$
but must replace $V$ by a line:
 \begin{center}\includegraphics[alt={A plane U and a line V through it.},scale=.3]{\gramSchmidtPath/Uo+V.jpg}\end{center}
...
The orthogonal complement $U^\perp$ is the subspace made from all vectors perpendicular to any vector in $U$. Here, we need to just tilt the line $V$ above until it hits $U$ at a right angle:
 \begin{center}\includegraphics[alt={A plane U and the perpendicular line U⊥.},scale=.3]{\gramSchmidtPath/U+Ut.jpg}\end{center} 
Notice, we can apply the same operation to $U^\perp$ and just get $U$ back again, {\itshape i.e.}
\[
