...
%What do you suspect about the vector space $\spa \{ \sin(ax)~|~ a \in \mathbb{R} \}$?
\item 
\begin{enumerate}
\item
...
%\emph{Hint: Note that $\dimension S_n = \dimension U_n$ where $U_n$ is the vector space of all $n \times n$ upper triangular matrices, and also note that $\dimension A_n = \dimension \widetilde{U}_n$ where $\widetilde{U}_n$ is the vector space of all strictly $n \times n$ upper triangular matrices (\emph{i.e.} the diagonal entries are all 0).}
