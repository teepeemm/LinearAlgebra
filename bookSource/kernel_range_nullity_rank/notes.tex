...
\]
we have 
\[ M^{-1} U = \left\{   a \colvec{2\\1\\0} +b \colvec{-1\\-1\\ 1}\,  \middle|\,  a\in [0,1] ,b\in \mathbb{R}\right\},\]
...
\begin{center}
\includegraphics[alt={The function f takes 3 points in S to two points in f(S)=T.},scale=.25]{onto.jpg}
\end{center} 

If \(f\) is both injective and surjective, it is {\bfseries bijective} (or an isomorphism.)
\begin{center}
\includegraphics[alt={The function f takes 5 points in S to five points in f(S)=T.},scale=.25]{biject.jpg}
\end{center} 
\begin{theorem}
...
Now let us specialize to functions \(f\) that are linear maps between two vector spaces. Everything we said above for arbitrary functions is exactly the same for linear functions. However, the structure of vector spaces let's us say much more about one-to-one and onto functions whose domains are vector spaces than we can say about functions on general sets.  For example, we know that a linear function always sends $0_V$ to $0_W$, {\itshape i.e.},
...
The notation for these numbers is 
 \Shabox{1}{$\nul L:=\dimension \ker L$,}  
 \Shabox{1}{$\rank L:=\dimension L(V) = \dimension\range L$.} 
...
%let \[L \colon \mathbb{R}^n \to \mathbb{R}^n\] be the linear transformation defined by \(L(v)=Mv\). Then 
