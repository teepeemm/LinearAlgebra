...
\item 
Consider an arbitrary matrix $M:\mathbb{R}^m\to \mathbb{R}^n.$
\begin{enumerate}
...
\item Argue further $\mathbb{R}^m=\ker M \oplus \range M^T$. 
\item Argue analogously that $\mathbb{R}^n=\ker M^T \oplus \range M$. 
\end{enumerate} 
The equations in the last two parts describe how a linear transformation $M:\mathbb{R}^m\to \mathbb{R}^n$ determines orthogonal decompositions of both its domain and target. This result sometimes goes by the humble name {\bf{The Fundamental Theorem of Linear Algebra}}\index{Fundamental Theorem of Linear Algebra}.
...
\item Suppose $L \colon \mathbb{R}^4\rightarrow \mathbb{R}^3$ whose matrix $M$ in the standard basis is row equivalent to the following matrix:
\[
\begin{pmatrix}
1&0&0&\!-1\\
0&1&0&1\\
...
\]
\begin{enumerate}
\item \emph{Explain} why the first three columns of the original matrix $M$ form a basis for $L(\mathbb{R}^4)$.
\item \emph{Find and describe} an algorithm (\textit{i.e.}, a general procedure) for computing a basis for $L(\mathbb{R}^n)$ when $L \colon \mathbb{R}^n\rightarrow \mathbb{R}^m$. 
\item{\itshape Use} your algorithm to find a basis for \(L(\mathbb{R}^4)\) when \(L \colon \mathbb{R}^4 \to \mathbb{R}^3\) is the linear transformation whose matrix \(M\) in the standard basis is
\[
\begin{pmatrix}
2&1&1&4\\
0&1&0&5\\
...
\item %(Extra credit) 
Let's demonstrate some ways the dimension formula can break down if a vector space is infinite dimensional.
\begin{enumerate}
...
\item[Note:] We could ask the same question for real vectors: If I choose a real vector at random, what is the probability that it lies in the span of some other vectors? In fact, once we write down a reasonable way of choosing a random real vector, if I choose a real vector in \(\mathbb{R}^n\) at random, the probability that it lies in the span of \(n-1\) other real vectors is zero! 
\end{itemize}
