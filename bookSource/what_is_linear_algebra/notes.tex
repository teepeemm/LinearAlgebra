...
% a little better let's try some examples. 
...
\[f(x,y)=3x+5y\,.\]
But let's think carefully; what is the left hand side of this equation doing? 
...
\begin{center}
\includegraphics[alt={A machine f that takes in x and outputs f(x).},scale=.3]{\whatIsPath//machine.jpg}
\end{center}
...
\[x \raisebox{-11mm}{\includegraphics[alt={An unlabeled machine with inputs and outputs.},scale=.14]{\whatIsPath/machine_blank.jpg} }\! \! \!\! \! \!10 x\, \hspace{8mm}\]
...
\[\!\!\!\!\!\!\!\!\!\colvec{x\\ y\\ z} \raisebox{-11mm}{\includegraphics[alt={An unlabeled machine with inputs and outputs.},scale=.14]{\whatIsPath/machine_blank.jpg} }\! \! \!\! \! \!\ccolvec{\phantom{-}z\\-z\\\!y-x\!\!}\, ,\]
...
\[\quad p \raisebox{-11mm}{\includegraphics[alt={An unlabeled machine with inputs and outputs.},scale=.14]{\whatIsPath/machine_blank.jpg} }\! \! \!\! \! \!\colvec{\int_{-1}^1  p(y) dy\\[3mm]
\int_{-1}^1 y p(y) dy}\, .\]
...
\begin{center}
\includegraphics[alt={A blob on the left with points cu, u, u+v, and v.  The function L takes these points to L(cu), L(u), L(u+v), and L(v), respectively, in a blob on the right.},scale=.4]{\whatIsPath/L.jpg}
\end{center}
...
%\end{enumerate}
\begin{center}
\includegraphics[alt={Other names for linear functions are "linear operator", "homomorphism", "linear map", and "linear transformation".},scale=.37]{\whatIsPath/Names.jpg}
\\
...
\begin{center}
\includegraphics[alt={As an example, four bags and five boxes of apples and bananas.},scale=.25]{\whatIsPath/boxesbags.jpg}
\end{center}
...
%x&=1 \,.
%\end{align*}
%That is, let's keep the variables on the left hand side and rewrite our {\itshape system} of equations after each step. \\
%
%
%\noindent
...
%Let's work toward reducing the amount of things we rewrite by combining the pair of equations in example 3 into a singe equation using vectors from 2-space. 
...
\end{center}
To exemplify, let's look at the derivative operator again.
...
\begin{center}
{\includegraphics[alt={As explained, suppose we have a problem Lu=v with a linear function L and vector V where it is too hard to solve to find u.  We can use a notation convention to transform to a matrix equation Mx=B, solve this to find solution(s) x, and reverse the convention to find the solution u.},width=.8\textwidth]{\whatIsPath/detourAbs} }\\
\end{center}
...
%We hope this helps you understand why we will be looking at matrices so much even though they will not be our primary objects of study. But, unfortunately we cannot start at the climax of our story; 
...
%
%To hint at what we meant by ``structure of matrices" let's look back on some math you already know.  
...
\begin{center}% keep my default of header-columns={1}
\begin{tabular}{|c|c|}
\hline
Logic &
\hwref{Background}{1}\\ 
...
%%\!\!\!\!\!\!\!\!\!\!\!\!\!\!\stackrel{\mathrm{ \begin{array}{c}\scriptstyle\text{Using~the~rule}\\[-2mm] \scriptstyle\text{for~cross~products~of}\\[-2mm]
%%\scriptstyle\text{scalar~multiples}\\\downarrow\\[1mm]\end{array}}}= 
%%\!\!\!\!\!\!\!\!\!\!\!\!\!\!c\colvec{1 \\ 1\\ 0} \times v=c\, Bv$\, .\\
%%\item~$B(u+v)=\colvec{1 \\ 1\\ 0} \times (u+v)
%%\!\!\!\!\!\!\!\!\!\!\!\!\!\!\stackrel{\mathrm{\begin{array}{c}\scriptstyle\text{Using~the~rule}\\[-2mm] \scriptstyle\text{for~cross~products}\\[-2mm]
%%\scriptstyle\text{of~sums}\\\downarrow\\[1mm]\end{array}}}
...
%
%
%%It is one thing to be able to tell which operators are linear and which are not, and it is another to have a feel for what linear operators do. Let's look at three simple example problems to get a feel for this. On the way we will introduce the main tool of linear algebra, the idea of a matrix. \\
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
