...
This can be intimidating, so have a pencil and paper ready so that you can write out some of the ideas yourself. Ready? Let's split the question ``What is Linear Algebra" in two: ``What is algebra?" and  ``What is linear?"\\  
...
One way to learn about them is by studying {\itshape systems of  linear equations}. 
...
We will do that in the Chapter~\ref{\gaussElimPath}. Let's try to use less ink by writing our system of  equations as a single equation between 2-vectors.
...
Section 1.1. 
Let's call any equation of the form~$Mu=v$ with~$M$ a matrix, and~$u,v$ vectors a {\itshape matrix equation}\index{matrix equation}. The first part of this course is dedicated to the study of matrix equations. 
