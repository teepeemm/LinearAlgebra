...
\[\begin{pmatrix}x\\y\\z\end{pmatrix}\times\begin{pmatrix}x'\\y'\\z'\end{pmatrix}:=\begin{pmatrix}yz'-zy'\\zx'-xz'\\xy'-yx'\end{pmatrix}\, .\] Indeed, $3$-vectors are special, usually vectors can only be added, not multiplied.

Let's find the force $F$ (a vector)  one must apply to a wrench lying along the vector 
