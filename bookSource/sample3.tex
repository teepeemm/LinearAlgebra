...
\item {\itshape Kirchoff's laws}: \index{Kirchoff's laws} Electrical circuits are easy to analyze using systems of equations.
...
\[
\includegraphics[alt={A circuit: a current of I amps flows from a battery of 60V, a current of 13A from s from a battery of 80V, and a current of J amps flows from a battery of V volts.  A resistor of 1 Ohm connects the outputs of the first two batteries; a resistor of 3 Ohms connects their inputs.  A resistor of 2 Ohms connects the outputs of the second two batteries; a resistor of 3 Ohms connects their inputs.},scale=.6]{circuit.pdf}
\]
...
\item
{\itshape Vandermonde determinant:}\index{Vandermonde determinant} Calculate the following determinants
\[
...
\item{\itshape Conic Sections}:\index{Conic sections} The equation for the most general conic section is given~by
\[
...
Let us say that in year zero, the Queen has no doves. In year one she has one pair of doves, in year two she has two pairs of doves {\itshape etc...}
...
\item Explain why (for any $n\geq 2$) the following {\itshape recursion relation}\index{Recursion relation}  holds
\[
F_n=F_{n-1}+F_{n-2}\, .
\]
...
\item Diagonalize $M$. ({\itshape I.e.}, write $M$ as a product $M=PDP^{-1}$ where $D$ is diagonal.)
\item Find a simple expression for $M^n$ in terms of $P$, $D$ and $P^{-1}$.
\item Show that $X_{n+1}=M^n X_1$.
\item The number
\[\varphi=\frac{1+\sqrt{5}}2 \]
is called the {\itshape golden ratio}\index{Golden ratio}. Write the eigenvalues of $M$ in terms of~$\varphi$.
...
Since this (spanning) set has three (linearly independent) elements, the vector space of linear maps $B^3\to B$ has dimension~3. This is an example of a general notion called the {\itshape dual vector space}\index{Dual vector space}.\end{enumerate}
...
\item The rows of $M$ span $(\ker L)^\perp$% in the basis given for $V$.
...
%\rank L=\dimension L(V)=\dimension W.
%\]
%So \[\dimension V=\rank L=\dimension W.\]
%\item Since \(\dimension V=\dimension W\), the matrix \(M\) is square so we can talk about its eigenvalues. Since \(L\) is injective, its kernel is the zero vector alone. That is, the only solution to \(LX=0\) is \(X=0_V\). But \(LX\) is the same as \(MX\), so the only solution to \(MX=0\) is \(X=0_V\). So \(M\) does not have zero as an eigenvalue.
%\item Since \(MX=0\) has no non-zero solutions, the matrix \(M\) is invertible.
%\end{enumerate}
%\item Now we suppose that \(M\) is an invertible matrix.
