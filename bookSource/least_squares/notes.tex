...
\vspace{-.1cm}
\begin{center}
\includegraphics[alt={The function L takes U to L(U) in W.  The point v is in W but not in L(U).},scale=.24]{notinimage.jpg}
\end{center} 
\vspace{-1.8cm}
...
Let's compute the angles between and lengths of these vectors. 
...
\]
Now let's compute the matrix of $L$ with respect to the orthonormal basis $O=(u_1,\ldots,u_n)$ for $V$ and the orthonormal basis~$O'=(v_1,\ldots,v_m)$ for~$W$. As usual, our starting point is the computation of $L$ acting on the input basis vectors;
\begin{align*}
...
\shabox{\centering
\bfseries \hyperref[sample3]{Congratulations, you have reached the end of the book! \\[2mm]
\includegraphics[alt={A student taking a final exam.},scale=.15]{final.jpg}\\
