...
Let's see how this works for small matrices first.
...
\begin{definition}
The {\bfseries sign function}\index{Sign function} is a function $\sgn $ that sends permutations to the set $\{-1,1\}$ with rule of correspondence defined by
\[ \sgn (\sigma) = 
\left\{ \begin{array}{rl}
...
\]
\shabox{\centering The determinant of a diagonal matrix is\\  the product of its diagonal entries.}
...
Let's \hypertarget{rowswap}{swap}
...
\begin{figure}
\begin{center}
\includegraphics[alt={Theorem: If M and M' differ by a row swap, then det M'=-det M.},scale=.3]{\elemMatDetPath/row_swap_theorem.jpg}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[alt={Theorem: If M'=M with a row multiplied by λ, then det M'=λ det M.},scale=.27]{\elemMatDetIIPath/row_mult_thm.jpg}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[alt={Theorem: If M' equals M with a multiple of one row added to another, then the determinants are the same.},scale=.27]{\elemMatDetIIPath/row_addition_thm.jpg}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[alt={Theorem: M is invertible if and only if det M ≠ 0.},scale=.27]{\elemMatDetIIPath/theorem_invertible.jpg}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[alt={det MN=det M det N},scale=.27]{\elemMatDetIIPath/detMN.jpg}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[alt={det M^T = det M},scale=.25]{\propDetPath/detMT.jpg}
\end{center}
...
\begin{center}
\includegraphics[alt={det M^{-1}=1/det M.},scale=.28]{\propDetPath/detMm1.jpg}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[alt={(adj M)M = (det M) I},scale=.3]{\propDetPath/adjM.jpg}
\end{center}
\end{figure}
...
Given three vectors $u,v,w$ in $\mathbb{R}^3$, the parallelepiped\index{Parallelepiped} determined by the three vectors is the ``squished'' box whose edges are parallel to $u, v$, and $w$ as depicted in Figure~\ref{parallelepiped}.
